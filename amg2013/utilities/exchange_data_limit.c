...
{  
	...
	int *completed_indexes;
	int num_completed_response, num_pending_response;

    int curr_contact_idx;
    int end_contact_idx;
    ...
    if (REQ_LIMIT == 0) {
	   	s = getenv("REQ_LIMIT");
		REQ_LIMIT = (s == NULL ? 100 : atoi(s));
		if (myid == 0)
			printf("REQ_LIMIT = %d.\n", REQ_LIMIT);
    }
	const int MAX_NUM_REQUEST = (num_contacts > REQ_LIMIT) ?
		REQ_LIMIT : num_contacts;

 	/* ---------initializations ----------------*/ 

//	if (myid == 33 && MAX_NUM_REQUEST < num_contacts)
//		printf("\n Rank 33 has %d messages to send.\n", num_contacts);
	...
	num_pending_response = 0;
    curr_contact_idx = 0;
    end_contact_idx = 0;
	if (num_contacts > 0 )
	{
		response_requests = hypre_CTAlloc(MPI_Request, MAX_NUM_REQUEST);
		response_statuses = hypre_CTAlloc(MPI_Status, MAX_NUM_REQUEST);
		completed_indexes  = hypre_CTAlloc(int, MAX_NUM_REQUEST);
		...
		curr_contact_idx = myid * 32051 % num_contacts;
        end_contact_idx = curr_contact_idx;
        num_pending_response = MAX_NUM_REQUEST;

		for (i=0; i < MAX_NUM_REQUEST; i++) 
		{
			/* response_ptrs[i] =  initial_recv_buf + i*max_response_total_bytes ; */
			response_ptrs[i] =  (void *)((char *) initial_recv_buf + 
				curr_contact_idx*max_response_total_bytes) ; 

			MPI_Irecv(response_ptrs[i], max_response_total_bytes, MPI_BYTE, 
				contact_proc_list[curr_contact_idx], 
				response_tag, comm, &response_requests[i]);
		
			contact_ptrs[curr_contact_idx] = (void *) ((char *) contact_send_buf + (
				(contact_send_buf_starts[curr_contact_idx] 
						- contact_send_buf_starts[0])
				*contact_obj_size)); 
			size = contact_send_buf_starts[curr_contact_idx+1] 
					- contact_send_buf_starts[curr_contact_idx]; 
			MPI_Isend(contact_ptrs[curr_contact_idx], size*contact_obj_size, MPI_BYTE,
				contact_proc_list[curr_contact_idx], 
				contact_tag, comm, &contact_requests[curr_contact_idx]); 
	
//	if (myid == 33 && MAX_NUM_REQUEST < num_contacts)
//		printf("Rank 33 has sent message to rank %d.\n", contact_proc_list[curr_contact_idx]);


			curr_contact_idx = (curr_contact_idx+1) % num_contacts;
		}
	}
	...
	while (!terminate)
	{
		...
		if (num_pending_response > 0) 
		{
			ierr = MPI_Testsome(num_pending_response, response_requests, 
				&num_completed_response, completed_indexes, response_statuses);
			int lastIdx = num_pending_response - 1;
			num_pending_response -= num_completed_response;

//	if (myid == 33 && MAX_NUM_REQUEST < num_contacts && num_completed_response > 0)
//		printf("Rank 33 has completed %d messages.\n", num_completed_response);


			while (num_completed_response > 0) {
				int deleteIdx = completed_indexes[--num_completed_response];
				if (deleteIdx != lastIdx)
					response_requests[deleteIdx] = response_requests[lastIdx];
				lastIdx--;
			}

			while (num_pending_response < MAX_NUM_REQUEST 
				&& curr_contact_idx != end_contact_idx)
			{
				/* response_ptrs[i] =  initial_recv_buf + i*max_response_total_bytes ; */
				response_ptrs[num_pending_response] =  (void *)((char *) initial_recv_buf + 
						curr_contact_idx*max_response_total_bytes) ; 

				MPI_Irecv(response_ptrs[num_pending_response], max_response_total_bytes, MPI_BYTE, 
						contact_proc_list[curr_contact_idx], 
						response_tag, comm, &response_requests[num_pending_response]);
...
//		printf("Rank 33 has sent message to rank %d.\n", contact_proc_list[curr_contact_idx]);
				...
				curr_contact_idx = (curr_contact_idx+1) % num_contacts;
				num_pending_response++;
			}

			if (num_pending_response == 0 && num_procs == 1) terminate = 1; /*added 11/08 */
		}
