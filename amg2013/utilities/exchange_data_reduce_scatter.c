...
{  
	...
	int *response_indexes;
	int response_count, response_remain;
	...
	// Use MPI_Reduce_scatter to get the number of messages to receive.

	int num_recv_messages;
	int *send_buf = hypre_TAlloc(int, num_procs);
	memset(send_buf, 0, sizeof(int) * num_procs);
	for (i = 0; i < num_contacts; i++)
		send_buf[contact_proc_list[i]]++;
	MPI_Reduce_scatter_block(send_buf, &num_recv_messages, 1, MPI_INT,
			MPI_SUM, comm);
	...
	if (myid%100 == 0) {
        char buf[10000];
        for (i = 0; i < num_contacts; i++)
			sprintf(buf + i * 5, "%4d,", contact_proc_list[i]);
        
		printf("Rank %4d sending %4d messages, receiving %4d messages. Sending to %s.\n", 
			myid, num_contacts, num_recv_messages, buf);
//		printf("Rank %4d sending %4d messages, receiving %4d messages.\n", 
//			myid, num_contacts, num_recv_messages);
	}
	/* ---------initializations ----------------*/ 
	...
	while (num_recv_messages > 0 || response_remain > 0)
	{

		/* did I receive any contact messages? */
		if (num_recv_messages > 0) {
		...
		if (response_remain > 0) 
		{
			ierr = MPI_Testsome(response_remain, response_requests, &response_count, 
					response_indexes, response_statuses);
			int lastIdx = response_remain - 1;
			response_remain -= response_count;

			while (response_count > 0) {
				int deleteIdx = response_indexes[--response_count];
	...
	/* end of (!response_remain > 0terminate) loop */
